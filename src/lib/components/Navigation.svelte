...
<nav class="flex h-32 items-center bg-surface-100-800-token px-12 py-6">
	<a href={url('/')}>
		<img
			src={url('/logo.png')}
			width={112}
	...
	<div class="hidden md:flex">
		<div class="ml-10 mr-6 h-20 w-px border-l border-surface-600-300-token" />

		{#each menus as menu, index}
			<div class="flex items-center">
				<a href={url(menu.path)} class="px-2 py-2 font-sans font-bold unstyled">{menu.name}</a>
				{#if index !== menus.length - 1}
					<div class="mx-6 h-14 w-px border-l border-surface-600-300-token" />
				{/if}
			</div>
		{/each}
	</div>
	...
	<div class="ml-auto hidden items-center md:flex">
		<div class="ml-12 mr-12 h-20 w-px border-l border-surface-600-300-token" />
	...
	<div class="ml-auto block md:hidden">
		<button
			class="flex items-center rounded border border-surface-600-300-token px-3 py-2"
			on:click={() => (open = !open)}
		>
	...
	{#if open}
		<div class="fixed inset-0 z-10 bg-surface-900 px-12 pb-6">
			<div class="flex h-32 items-center">
				...
				<button
					class="ml-auto flex items-center rounded border border-surface-600-300-token px-3 py-2"
					on:click={() => (open = !open)}
				>
					<svg class="h-3 w-3 fill-current" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
						<title>Close</title>
			...
			<div class="mt-5 flex h-4/5 flex-col items-center">
				{#each menus as menu}
					<a
						href={url(menu.path)}
						class="px-2 py-2 font-sans font-bold unstyled"
