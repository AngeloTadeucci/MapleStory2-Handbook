<script lang="ts">
  ...
  function validate(): boolean {
    ...
    // State is valid
    if (defaultValue === input) return false;
    return true;
  }
...
<div class="input-chip {classesBase}" class:opacity-50={$$restProps.disabled}>
  ...
  <div class="h-0 overflow-hidden">
    <select bind:value {name} multiple {required} tabindex="-1">
      ...
      {#each value as option}<option class="" value={option}>{option}</option>{/each}
    </select>
  </div>
  <!-- Interface -->
  <div class="input-chip-interface {classesInterface}">
    ...
    <form on:submit={addChip}>
      ...
      >
        {#each whitelist.filter((x) => !chipValues.map((y) => y.val).includes(x)) as option}<option
